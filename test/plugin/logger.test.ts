...
describe("Logger 日志插件", () => {
  ...
  describe("afterAction - Action 执行后", () => {
    ...
    it("showDuration 为 true 时应该记录耗时", () => {
      ...
      const calls = mockLogger.log.mock.calls;
      const hasDuration = calls.some((call: any[]) => typeof call[0] === "string" && call[0].includes("⏱️ 耗时:"));
  ...
  describe("集成测试", () => {
    ...
    it("应该能够处理多个 action 的顺序执行", async () => {
      ...
      const actionCalls = groupCalls.filter((call: any[]) => call[0] && call[0].includes("⚡ Action:"));
    ...
    it("禁用 logDataChanges 时不应该记录数据变更日志", async () => {
      ...
      // 不应该有数据变更相关的日志
      const groupCalls = mockLogger.group.mock.calls;
      const dataChangeCalls = groupCalls.filter((call: any[]) => call[0] && call[0].includes("📦 数据变更:"));
