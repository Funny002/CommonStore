...
describe('ReduxDevtools 插件', () => {
  ...
  describe('install - 安装插件', () => {
    it('安装时应该连接 DevTools 并发送初始状态', () => {
      ...
      expect(ext.connect).toHaveBeenCalledWith(expect.objectContaining({
        name: 'TestStore',
        maxAge: 50,
        latency: 0,
      }));
  ...
  describe('onError - 错误处理', () => {
    it('action 出错时应发送错误到 DevTools', async () => {
      ...
      expect(mockConnection.error).toHaveBeenCalledWith(
        'Action "broken" failed: Something broke',
      );
    });
  });
