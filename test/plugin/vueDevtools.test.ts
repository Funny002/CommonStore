...
describe('VueDevtools 插件', () => {
  ...
  describe('install - 安装插件', () => {
    ...
    it('收到 init 事件后应设置 Inspector 和 Timeline', async () => {
      ...
      expect(mockApi.addInspector).toHaveBeenCalledWith(expect.objectContaining({
        id: 'common-store',
        label: 'MyStore',
        icon: 'storage',
      }));

      expect(mockApi.addTimelineLayer).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('Timeline - 时间线事件', () => {
    it('beforeAction 应发送 timeline 事件', async () => {
      ...
      expect(mockApi.addTimelineEvent).toHaveBeenCalledWith(expect.objectContaining({
