...
describe("Store", () => {
  ...
  describe("dispatch (Action 执行)", () => {
    ...
    it("执行未注册的 action 应该抛出错误", () => {
      expect(store.dispatch("nonexistent")).rejects.toThrow('Action "nonexistent" not found');
    });
  ...
  describe("ActionManager 独立方法", () => {
    ...
    it("注销不存在的 action 应该抛出错误", () => {
      expect(() => store.actions.unregister("unknown")).toThrow('Action "unknown" is not registered');
    });
  ...
  describe("插件系统 (use)", () => {
    ...
    it("重复注册同一个插件应该抛出错误", () => {
      ...
      expect(() => store.use(plugin)).toThrow("Plugins already registered: duplicate-plugin");
    });
  ...
  describe("subscribe - 订阅状态变化", () => {
    ...
    it("订阅根路径（空字符串）应该抛出错误", () => {
      const callback = vi.fn();
      expect(() => store.subscribe("", callback)).toThrow("Cannot subscribe to the root path. Please specify a specific path.");
    });
  ...
  describe("边界情况", () => {
    ...
    it("插件缺少依赖时应该抛出明确错误", () => {
      ...
      expect(() => store.use(plugin)).toThrow('Missing dependency "notExist" for plugin "missingDep"');
    });

    it("包含空段的路径应该抛出错误", () => {
      const testStore = new Store({ a: { b: 1 } });
      expect(() => testStore.data.set("a..b", 2)).toThrow("path contains empty segments");
      expect(() => testStore.getState("a..b")).toThrow("path contains empty segments");
    });

    it("getRaw 空路径应该返回整个状态", () => {
      const testStore = new Store({ a: 1, b: 2 });
      expect(testStore.data.getRaw("")).toEqual(testStore.data.getRaw([]));
      expect(testStore.data.getRaw()).toEqual(testStore.data.getRaw([]));
    });
  });
});
